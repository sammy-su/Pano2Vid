...
def compute_distance(trajs_src, trajs_dst, metric="cosine"):
    ...
    for i in xrange(trajs_src.shape[0]):
        ...
        if metric == "cosine":
            ...
            threshold = np.pi * 65.5 / 180.
            dists = 1. - delta_angles / threshold
            dists[dists<0.] = 0.
        else:
...
def humanedit_metric(trajs, humanedits, pool='traj', metric='cosine'):
    results = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--metric', choices=['cosine', 'overlap'], default='cosine')
